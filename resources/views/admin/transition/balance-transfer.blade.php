        ...
        <div class="container-xl">
            <div class="row g-2 align-items-center">
                ...
                <div class="col-auto ms-auto d-print-none">
                    <div class="btn-list">
                        <a href="javascript:void(0);" class="btn btn-primary btn-5 d-none d-sm-inline-block" data-bs-toggle="modal"
                        ...
                        <table class="table datatable table-striped table-bordered">
                            ...
                            <tbody class="table-tbody">
                                @foreach($accountTransfers as $accountTransfer)
                                    ...
                                    @endphp
                                <tr>
                                    <td>{{$accountTransfer->fromAccount ? $accountTransfer->fromAccount->account_number : '' }}  -  {{ $accountTransfer->fromAccount && $accountTransfer->fromAccount->bank ? $accountTransfer->fromAccount->bank->bank_name : ''}}</td>
                                    <td>{{$accountTransfer->toAccount ? $accountTransfer->toAccount->account_number : '' }}  -  {{ $accountTransfer->toAccount && $accountTransfer->toAccount->bank ? $accountTransfer->toAccount->bank->bank_name : ''}}</td>
                                    <td>{{$from_account_currency}} {{$accountTransfer->amount}}</td>
                                    <td>{{$to_account_currency}} {{$accountTransfer->exchange_amount}}</td>
                                    <td>{{\Carbon\Carbon::parse($accountTransfer->transfer_date)->format('d/m/Y')}}</td>
                                    <td>{{$accountTransfer->note}}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
